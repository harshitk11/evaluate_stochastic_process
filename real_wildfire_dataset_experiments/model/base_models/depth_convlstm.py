...
class EncoderBottleneck(nn.Module):
    def __init__(self, args):
        ...
        DOWNSAMPLE_FACTOR = args.model.d_convLSTM.bottleneck_factor  

        # Encoder	
        self.conv1 = nn.Conv2d(in_channels=INPUT_CHANNELS, 
                               out_channels=OUTPUT_CHANNELS, 
                               kernel_size=KERNEL_SIZE, 
                               stride=DOWNSAMPLE_FACTOR)  # Using stride for downsampling
...
class DecoderBottleneck(nn.Module):
    def __init__(self, args):
        ...
        UPSAMPLE_STRIDE = args.model.d_convLSTM.bottleneck_factor  
        FINAL_OUTPUT_SIZE = args.dataloader.image_spatial_size  # Desired final output size
        UPSAMPLE_MODE = 'bilinear'  
        
        # Decoder
        self.convT1 = nn.ConvTranspose2d(in_channels=INPUT_CHANNELS, 
                                         out_channels=OUTPUT_CHANNELS, 
                                         kernel_size=KERNEL_SIZE, 
                                         stride=UPSAMPLE_STRIDE)  # Stride for learnable upsampling
                                         
        self.relu = nn.ReLU()
        self.upsample = nn.Upsample(size=FINAL_OUTPUT_SIZE, mode=UPSAMPLE_MODE, align_corners=True)

    def forward(self, context):
        x = self.relu(self.convT1(context))  
        x = self.upsample(x)  
        return x
...
class depth_convlstm(nn.Module):
    def __init__(self, args):
        ...
        if self.BOTTLENECK_FACTOR > 1:
            print("******* Botlenecked ConvLSTM *******")
            self.encoder = EncoderBottleneck(args)
        else:
        ...
        # Decoder
        if self.BOTTLENECK_FACTOR > 1:
            self.decoder = DecoderBottleneck(args)
        # else:
