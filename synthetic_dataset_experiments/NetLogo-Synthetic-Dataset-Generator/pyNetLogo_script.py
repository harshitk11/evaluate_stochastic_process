...
class DensityOfStates:
    ...
    def generate_DOS_distribution(num_MC_runs=1000, density=80, stochastic_meter=0.50, save_path=None, heterogenous_agents=False,  scales=[1,2,4,8,16], debug=False):
        """
        Used to generate a distribution of the number of unburnt trees at the end of the simulation
        params:
            - num_MC_runs: Number of monte carlo runs to perform
            - density: Density of the forest
            - stochastic_meter: Stochasticity of the forest
        ...
        for i in range(num_MC_runs):
            ...
            if heterogenous_agents:
                ## Read the p_tef attribute and rewrite it as some gaussian (for heterogenous agents)
                patchAttr_df = netlogo_wrapper.read_patch_attribute(netlogoRun = netlogo_run, patchAttribute ='p_tef')    
                size_x, size_y = patchAttr_df.shape
                # patchAttr_df = generate_gaussian_mixture(width=size_x, height=size_y, save_path=None)
                patchAttr_df = create_steepnes_map(width=size_x, height=size_y, save_path=None, load_previous_data=True)
                netlogo_wrapper.write_patch_attribute(netlogoRun=netlogo_run, patchAttribute='p_tef', patchAttr_df=patchAttr_df)
                
            netlogo_run = netlogo_wrapper.run_netlogoSimulation_for_N_ticks(netlogoRun=netlogo_run, N=500)
            ...
            for scale in scales:
                scaled_patchAttr_df = DensityOfStates.maxpooling_2d(patchAttr_df, scale) if scale > 1 else patchAttr_df
                num_unburnt_trees = np.count_nonzero(scaled_patchAttr_df.values == 55)
                num_unburnt_trees_dict[i][scale] = num_unburnt_trees
            #     fpath = f"./images/forestmap/forestmap_{density}_{stochastic_meter}_{scale}_{i}.png"
            #     os.makedirs(os.path.dirname(fpath), exist_ok=True)
            #     visualization_tools.plot_netlogo_map(patchAttr_df=scaled_patchAttr_df, save_path=fpath)
            #     print(f" [Scale: {scale}] Number of unburnt trees: {num_unburnt_trees}")
            # exit()
            netlogo_wrapper.killNetlogo(netlogoRun=netlogo_run)

            if debug:
                input("Press Enter to continue...")
                os.system("rm -r /hdd2/extra_home/hkumar64/netlogo/behaviorspace_experiments/*")
