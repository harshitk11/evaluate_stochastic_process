...
def train_and_eval_model(config, epoch, model, optimizer, scheduler, trainloader, testloader, writer=None, log_file=None):
    ...
    if writer:
        ...
        if train_metrics['learned_multiscale_weights'] is not None:
            for i, weight in enumerate(train_metrics['learned_multiscale_weights']):
                writer.add_scalar(f'ScaleWeights/weight_{i}', weight, epoch)
    
    if log_file:
        log_file.write(f"Epoch {epoch}: Training Loss = {train_metrics['train_loss']}, Validation Loss = {eval_metrics['val_loss']}\n")
        if train_metrics['learned_multiscale_weights'] is not None:
            weights_str = ', '.join(map(str, train_metrics['learned_multiscale_weights']))
            log_file.write(f"Epoch {epoch}: Learned Multi-scale Weights = [{weights_str}]\n")
...
def main():
    ...
    start_epoch = 0
    # CHECKPOINT_PATH = log_dir # For resuming from the last checkpoint
    if RESUME and (CHECKPOINT_PATH != 'null'):
    ...
    for epoch in range(start_epoch, EPOCHS):
        train_metrics, eval_metrics = train_and_eval_model(config, epoch, model, optimizer, scheduler, trainloader, testloader, writer, log_file)
