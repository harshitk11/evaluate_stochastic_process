...
from .base_models.ar_nca import ForestFirePredictor_ARNCA
...
class BaseModel_WithMultiScale(nn.Module):
    def __init__(self, config, base_model, writer=None):
        ...
        elif base_model == "arnca":
            config.mode = config.model.arnca.mode
            config.device = config.dataloader.device
            self.base_model = ForestFirePredictor_ARNCA(config)
    ...
    def forward(self, x):
        ...
        if self.base_model_name == "arnca":
            x_pred = self.base_model(x, location=None)
            # Convert shape from T,B,H*W,C to T,B,C,H,W
            x_pred = x_pred.reshape(59,-1,64,64).unsqueeze(2)
            # Convert shape from T,B,C,H,W to B,T,C,H,W
            x_pred = x_pred.permute(1,0,2,3,4)
            x_ms_t = [x_pred]
            return x_ms_t
        else:
